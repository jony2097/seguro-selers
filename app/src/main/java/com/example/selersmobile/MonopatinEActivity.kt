...
class MonopatinEActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnConfirmar.setOnClickListener {
            val patent = generarPatente("MONOPATIN")
            val idGenerado = generarNumeroPoliza()
            val vigencia = generarFechaVigencia()

            when {
                radioParcial.isChecked -> {
                    guardarSeguro("MONOPATIN", "Plan Parcial", idGenerado, patent, vigencia)
                    mostrarAlertaExito("Plan Parcial", idGenerado)
                }
                radioTotal.isChecked -> {
                    guardarSeguro("MONOPATIN", "Plan Total", idGenerado, patent, vigencia)
