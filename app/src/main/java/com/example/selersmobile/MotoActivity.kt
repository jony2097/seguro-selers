...
class MotoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_motocicleta)
        ...
        btnConfirmar.setOnClickListener {
            val patent = generarPatente("MOTOCICLETA")
            val idGenerado = generarNumeroPoliza()
            val vigencia = generarFechaVigencia()

            when {
                radioParcial.isChecked -> {
                    guardarSeguro("MOTOCICLETA", "Plan Parcial", idGenerado, patent, vigencia)
                    mostrarAlertaExito("Plan Parcial", idGenerado)
                }
                radioTotal.isChecked -> {
                    guardarSeguro("MOTOCICLETA", "Plan Total", idGenerado, patent, vigencia)
